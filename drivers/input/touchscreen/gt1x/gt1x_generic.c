...
#define PINCTRL_STATE_ACTIVE	"pmx_ts_int_active"
...
s32 gt1x_init_panel(void)
{
	...
	GTP_INFO("gt1x_config=%x\n",gt1x_config[0]);
	buf[3] =gt1x_config[0];
	GTP_INFO("gt1x_config=%x\n",buf[3]);
	CURRENT_FIRMWARE_ID = (buf[0]<<24) | (buf[1]<<16) | (buf[2]<<8) | buf[3];
	GTP_INFO("CURRENT_FIRMWARE_ID = 0x%x\n", CURRENT_FIRMWARE_ID);
	tp_info.manufacture ="GT1151";
	tp_fw = CURRENT_FIRMWARE_ID;
	sprintf(fw_id,"0x%x",tp_fw);
	tp_info.version ="0X47";
...
int gt1x_ts_pinctrl_init(struct i2c_client *client)
{
	int retval;
	int err;
	...
	if (ts_pinctrl) {
		err = pinctrl_select_state(ts_pinctrl,
					pinctrl_state_active);
		if (err < 0)
			GTP_INFO("Cannot get suspend pinctrl state\n");
	}
