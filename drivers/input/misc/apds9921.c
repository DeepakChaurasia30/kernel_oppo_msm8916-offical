...
#ifdef VENDOR_EDIT  
/*xiaohua.tian@Prd6.BaseDrv.Sensor,2016/08/20 add for control for apds9922*/
#define APDS9922_DD_PRX_DEFAULT_PULSE           28
#endif
...
#ifdef VENDOR_EDIT //xiaohua.tian@EXP.driver,add for light value for 15399, 2016-03-12 
extern int lcd_dev;
enum {
...
static int Lux_original(struct i2c_client *client, int als_data)
{
	...
	if (is_project(OPPO_15018))
		...
		als_data = als_data * 4;
	#ifdef VENDOR_EDIT //xiaohua.tian@EXP.driver,add for light value for 15399, 2016-01-26 
	else if (is_project(OPPO_15399) && ( LCD_15399_TRULY_NT355521S == lcd_dev))
		als_data = als_data * 22/10;		
	if (is_project(OPPO_15399)&&( LCD_15399_BOE_ILI9881C == lcd_dev || LCD_15399_TM_NT355521S == lcd_dev))
		als_data = als_data * 38/10;		
	#endif
...
/*xiaohua.tian@Prd6.BaseDrv.Sensor,2016/08/20 add for control for apds9922*/
	...
	{
		err = apds9921_dd_set_prx_led(client, APDS9921_DD_LED_FREQ_80_KHZ|APDS9921_DD_PRX_DEFAULT_LED_CURRENT);
		if (err < 0) 
			return err;

		err = apds9921_dd_set_prx_pulses(client, APDS9922_DD_PRX_DEFAULT_PULSE);
		if (err < 0) 
			return err;
	}
#endif
...
static ssize_t apds9921_name_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	#ifdef VENDOR_EDIT  
	/*xiaohua.tian@Prd6.BaseDrv.Sensor,2016/08/20 add for control for apds9922*/
	int id = i2c_smbus_read_byte_data(apds9921_i2c_client, APDS9921_DD_PART_ID_ADDR);
	if (id == 0xB3)
		return snprintf(buf, PAGE_SIZE, "%s", "apds9922");
	else
		...
		const struct i2c_device_id *id)
{
	...
	#ifdef VENDOR_EDIT  
	/*xiaohua.tian@Prd6.BaseDrv.Sensor,2016/08/20 add for control for apds9922*/
	devinfo = i2c_smbus_read_byte_data(apds9921_i2c_client, APDS9921_DD_PART_ID_ADDR);
	pr_err("%s: xiaohua the alsps is : %x\n", __func__, devinfo);
	if (devinfo == 0xB3)
	{
		data->als_cdev.name = "apds9922-light";
		data->ps_cdev.name = "apds9922-proximity";
	}	
	#endif
