...
/*rongchun.zhang@EXP.BaseDrv 2016-01-20 add for factory mode sleep*/
static int disp_enp_gpio_15399 = 999 ;		//GPIO_97
static int disp_enn_gpio_15399 = 1000;		//GPIO_98
/*rongchun.zhang@EXP.BaseDrv END*/
#endif /*VENDOR_EDIT*/
...
/* YongPeng.Yi@SWDP.MultiMedia, 2015/06/13  Add for add for cabc filter strength START */
    ...
    pr_err("%s REG_CTRL:0x%x\n", __func__, reg_val);
        /* Le.Li@EXP.MultiMedia.Displa.LCD.Feature, 2017/05/04, Modify for 15399 with new backlight ic SM5306. */
...
/* YongPeng.Yi@SWDP.MultiMedia END */
		...
		/* goushengjun@SWDP.MultiMedia END */
				...
				/*rongchun.zhang@EXP.BaseDrv 2016-01-20 add for factory mode sleep*/
	            if(MSM_BOOT_MODE__FACTORY == get_boot_mode()&&is_project(OPPO_15399)){
					mdelay(230);
					if(gpio_is_valid(disp_enp_gpio_15399))
						gpio_set_value(disp_enp_gpio_15399, 0); //GPIO_114
					else
						pr_err("%s: unvalid gpio disp_enp_gpio_15399 \n", __func__);
					mdelay(3);
					if(gpio_is_valid(disp_enn_gpio_15399))
						gpio_set_value(disp_enn_gpio_15399, 0); //GPIO_113
					else
						pr_err("%s: unvalid gpio disp_enn_gpio_15399 \n", __func__);
				}
				/*rongchun.zhang@EXP.BaseDrv END*/
#endif /*VENDOR_EDIT*/
...
#else /*VENDOR_EDIT*/
		if(!is_project(OPPO_15399)){ /*lile@EXP.BasicDrv.LCD, 2016-03-25, modify the brightness*/
			if(bl_level < 4)
				bl_level = 4;
		}
...
/*lile@EXP.BasicDrv.LCD, 2015-07-10,  Add for 15069 HD JDI minimum backlight*/
		...
		}else if(is_project(OPPO_15037)||is_project(OPPO_15035)||is_project(OPPO_15399)){
			if(cabc_mode == 3){
		...
		if(is_project(OPPO_15037) || is_project(OPPO_15035)|| is_project(OPPO_15399)){
...
/* Xiaori.Yuan@Mobile Phone Software Dept.Driver, 2014/09/23  Add for register backlight info */
	...
	/* YongPeng.Yi@SWDP.MultiMedia, 2015/10/08  Add for 15035 BL SM5306 */
        ...
        /* Le.Li@EXP.MultiMedia.Displa.LCD.Feature, 2017/05/04, Modify for 15399 with new backlight ic SM5306. */
	if((is_project(OPPO_15035)||is_project(OPPO_15109) || is_project(OPPO_15399)) && (revision == 0x00)){
		/* YongPeng.Yi@SWDP.MultiMedia, 2015/10/08  Add for 15035 BL SM5306 */
