...
#define IRQD_IRQ_DISABLEDGOODIX (1 << 16)
...
extern int gt1x_ts_pinctrl_init(struct i2c_client *client);
...
{
	//struct irq_desc *irq_goodix_handler;
	GTP_DEBUG_FUNC();
	//irq_goodix_handler = irq_to_desc(gt1x_i2c_client->irq);
				    	...
				    	ret = gt1x_i2c_read(GTP_REG_WAKEUP_GESTURE_DETAIL, coordinate_single, coordinate_size*4);
							...
							switch (doze_buf[0]) 
							{
								...
								if(((Point_input[1].x - Point_input[0].x)*(Point_input[2].y - Point_input[1].y)-(Point_input[1].y - Point_input[0].y)*(Point_input[2].x)-Point_input[1].x)>0)
									...
									clockwise =0;
								GetCirclePoints_15085(&Point_input[0], coordinate_size,Point_output);
...
static s32 gt1x_request_io_port(void)
{
	...
	irq_goodix = irq_to_desc(gt1x_i2c_client->irq);
	return ret;
}
	...
	if( 1 == is_in_suspend )
	{
		...
		switch(ret)
		{
			case 0:
				...
				GTP_INFO("is_in_suspend_before is_in_suspend= %d,gesture_enabled= %d\n",is_in_suspend,gesture_enabled);
				gt1x_enter_sleep_gesture();
				GTP_INFO("is_in_suspend_after is_in_suspend= %d,gesture_enabled= %d\n",is_in_suspend,gesture_enabled);
				break;
			case 1:
				gt1x_wakeup_sleep_gesture();
				GTP_INFO("is_in_suspend_before is_in_suspend= %d,gesture_enabled= %d\n",is_in_suspend,gesture_enabled);
...
#endif
	gt1x_ts_pinctrl_init(gt1x_i2c_client);
